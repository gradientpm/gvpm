...
struct MTS_EXPORT_BIDIR PathVertex {
	...
	*/
	...
	bool sampleNext(const Scene *scene, Sampler *sampler,
		...
		Spectrum *throughput = NULL, int componentSel = -1, bool longBeam = false, bool adjointComp = false);
	...
	bool perturbDirection(const Scene *scene, const PathVertex *pred,
		...
		const Vector &d, Float dist, EVertexType desiredType,
						  ETransportMode mode, bool adjointComp = false);
	...
	bool propagatePerturbation(const Scene *scene, const PathVertex *pred,
		...
		ETransportMode mode, bool adjointComp = false, int componentSel = -1);
	...
	Spectrum eval(const Scene *scene, const PathVertex *pred,
		const PathVertex *succ, ETransportMode mode, EMeasure measure = EArea, bool adjointComp = false) const;
