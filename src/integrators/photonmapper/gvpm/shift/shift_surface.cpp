...
//	  if(!generateShiftPathME(source, proposal, b, c, thdata.pool,
...
//	  }
  ...
  // Update photon location to last vertex of the proposed path
  // The difference is small though.
  // This should not matter
  //itsProj = proposal.vertex(c)->getIntersection();

  ShiftRecord sRecME;
  if (!ShiftME(sRecME, source, proposal, b, c)) {
    for (int i = b; i <= c; ++i) {
      thdata.pool.release(proposal.edge(i - 1));
      thdata.pool.release(proposal.vertex(i));
    }
  ...
  Float jacobian = result.jacobian;  // middle term | dy_s' / dy_s |
  ...
  Spectrum photonWeight = jacobian * sRecME.throughtput;
  ...
  if (config.useMIS) {
    ...
    Float allJacobian = shiftGather->surfInfo().jacobian * jacobian;
