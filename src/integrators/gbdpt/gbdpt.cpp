...
public:
    ...
    bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        while(true) {
            ...
            if(virtualIter % m_dumpIteration == 0) {
                ...
                // Write the bitmap
                develop(scene, hdrFilm, dxBitmap, virtualIter, "_dxAbs_");
                develop(scene, hdrFilm, dyBitmap, virtualIter, "_dyAbs_");
                develop(scene, hdrFilm, imgBitmap, virtualIter, "_throughput_");
                ...
                if(m_config.m_reconstructL2) {
                    ...
                    solverL2.importImagesMTS(&dxf[0], &dyf[0], &imgf[0],
                                             nullptr, film->getCropSize().x, film->getCropSize().y);
                    ...
                    if(m_config.forceBlackPixels) {
                        for(size_t p = 0; p < len; p+=3) {
                            if(imgf[p] == 0.f && imgf[p+1] == 0.f && imgf[p+2] == 0.f) {
                                rec[p] = rec[p+1] = rec[p+2] = 0.f;
                            }
                        }
                    }
                    setBitmapFromArray(recBuff, &rec[0], false, true);

                    if(m_config.m_reconstructL1) {
                        develop(scene, hdrFilm, recBuff, virtualIter, "_L2_");
                    } else {
                        develop(scene, hdrFilm, recBuff, virtualIter, "_recons_");
                    }

                    /* put L2 image on screen buffer */
                ...
                if(m_config.m_reconstructL1) {
                    ...
                    solverL1.importImagesMTS(&dxf[0], &dyf[0], &imgf[0],
                                             nullptr, film->getCropSize().x, film->getCropSize().y);
                    ...
                    if(m_config.m_reconstructL2) {
                        develop(scene, hdrFilm, recBuff, virtualIter, "_L1_");
                    } else {
                        develop(scene, hdrFilm, recBuff, virtualIter, "_recons_");
                    }
