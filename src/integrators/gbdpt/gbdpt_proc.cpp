...
public:
	...
	void prepare() {
		...
		m_offsetGenerator = new ManifoldPerturbation(m_scene, NULL, m_pool, 0.f, true, true, 0, 0,
																								 m_config.m_shiftThreshold,
																								 m_config.maxManifoldIterations);
	}
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult, const bool &stop) {
		...
		for (size_t i = 0; i<m_hilbertCurve.getPointCount(); ++i) {
			...
			size_t neighborCount = m_config.nNeighbours;
			...
			Path emitterSubpath; // Other subpaths are generated on the fly
...
//			Point2 samplePos;
			...
			size_t spp = m_sampler->getSampleCount();
			...
			for (size_t j = 0; j<spp; j++) {
				...
				Path::alternatingRandomWalkFromPixel(m_scene, m_sampler,
					emitterSubpath, emitterDepth, sensorSubpath[0],
...
//				samplePos = sensorSubpath[0].vertex(1)->getSamplePosition();
				...
				for (size_t k = 0; k < (1 + m_config.nNeighbours); ++k){
					const_cast<ImageBlock *>(result->getImageBlock(k))->setAllowNegativeValues(true);
					if (m_config.lightImage)
						const_cast<ImageBlock *>(result->getLightImage(k))->setAllowNegativeValues(true);
				}
				Path connectPath;
				int ptx;

				/* create shift-able path  */
				createShiftablePath(connectPath, emitterSubpath, sensorSubpath[0], 1, sensorSubpath[0].vertexCount() - 1, ptx);

				/*  geometry term(s) of base  */
				m_offsetGenerator->computeMuRec(connectPath, pathData[0].muRec);
				for (int v = pathData[0].muRec.extra[0] - 1; v >= 0; v--){ // v = first hit point
					size_t idx = connectPath.vertexCount() - 1 - v;
					if (Path::isConnectable_GBDPT(connectPath.vertex(v), m_config.m_shiftThreshold) && v >= pathData[0].muRec.extra[2]) {
						// Until that we didn't reach the the second diffuse vertex
				...
				for (size_t k = 0; k<neighborCount; k++){
					...
					if(pathData[0].muRec.extra[0] <= 2) {
						pathData[k + 1].success = false;
					} else {
						pathData[k + 1].success =  m_offsetGenerator->generateOffsetPathGBDPT(connectPath,
																							  sensorSubpath[k + 1],
																							  pathData[k + 1].muRec,
																							  shifts[k], // Offset
																							  pathData[k + 1].couldConnectAfterB,
																							  false);
					}
					...
					if (pathData[k + 1].success){
						...
						size_t idx = 0; int a, b, c;
						for (int v = pathData[k + 1].muRec.extra[0] - 1; v >= 0; v--){
							idx = connectPath.vertexCount() - 1 - v;
				...
				evaluate(result, emitterSubpath, sensorSubpath, pathData, v_b,
...
//		int pixelIndex = (int)initialSamplePos[0] + (int)initialSamplePos[1] * m_sensor->getFilm()->getSize().x;
		...
		// FIXME: No initialized but used?!
        bool successConnectBase;
		Float offsetImportancePdf;
		Point2 samplePos;
		...
		for (int s = (int)emitterSubpath.vertexCount() - 1; s >= 0; --s) {
			...
			if(m_config.minDepth > 0) {
				if((s+1) < m_config.minDepth) {
					continue;
				}
			}
...
//				samplePosValid = true;
                ...
                // This code part is to remove all surface rendering
				if(!(m_config.lightingInteractionMode & EAll2Surf)) {
					// --- Step 1: Build the full path
                    m_pathVertex.clear();
					// Check the type of path
					for(int idEye = 1; idEye <= t; idEye++) {
						m_pathVertex.push_back(sensorSubpath[0].vertex(idEye));
					}
					for(int idLight = s; idLight >= 1; idLight--) {
						m_pathVertex.push_back(emitterSubpath.vertex(idLight));
					}

                    if(!bdptDecisionVolume(m_pathVertex, m_config.minCameraDepth)) {
						continue;
					}
				}
				...
				/* Just a test to remove all not specular paths */
				...
				for (int k = 0; k <= neighbourCount; k++){
					...
					do{ //this should really go away at some point...
						if (pathSuccess[k] && pathSuccess[0] && (k == 0 || (valuePdf[0]>0 && !value[0].isZero()))){
							...
							int interactions = 0; // remaining; // backup
...
private:
	...
	void createShiftedLightPath(Path &base, Path &offset, double &jacobian, bool &pathSuccess, Spectrum &offsetWeight, Float &offsetPdf, MutationRecord &muRec,
		const Point2 samplePos, const Vector2 shift, const size_t s){
		jacobian = 1.0;
	...
	**/
	bool createShiftablePath(Path &connectedPath, Path &emitterSubpath, Path &sensorSubpath, int s, int t, int &memPointer){
		...
		connectedPath.edge(memPointer)->medium = sensorSubpath.edge(0)->medium;
...
private:
    ...
    // To build the path representation
    std::vector<PathVertex*> m_pathVertex;
};
