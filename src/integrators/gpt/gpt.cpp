...
struct NextSurfaceComponent {
  Float pdf;
  int component;
  bool isRought;
  NextSurfaceComponent(): pdf(1.0), component(-1), isRought(false) {
  }
};
...
struct RayState {
  ...
  Spectrum throughput = Spectrum(1.f);             ///< Current throughput of the path.
  ...
  Spectrum radiance = Spectrum(0.f);               ///< Radiance accumulated so far.
  Spectrum gradient = Spectrum(0.f);               ///< Gradient accumulated so far.

  RadianceQueryRecord rRec;        ///< The radiance query record for this ray.
  Float eta = 1.f;                       ///< Current refractive index of the ray.
  bool alive = true;                      ///< Whether the path matching to the ray is still good.
  ///< Otherwise it's an invalid offset path with zero PDF and throughput.
  bool lastVolume = false;

  RayConnection connection_status = RAY_NOT_CONNECTED; ///< Whether the ray has been connected to the base path, or is in progress.
...
struct HalfVectorShiftResult {
  ...
  Float jacobian = 1.f; ///< Local Jacobian determinant of the shift.
  Vector3 wo = Vector3(0.f);     ///< Tangent space outgoing vector for the shift.
};
...
//      result.success = false;
      result.success = true;
      result.wo = -tangentSpaceShiftedWi;
      result.jacobian = 1.f;
      return result;
    }
...
struct ReconnectionShiftResult {
  bool success = false;   ///< Whether the shift succeeded.
  Float jacobian = 1.f; ///< Local Jacobian determinant of the shift.
  Vector3 wo = Vector(0.f);     ///< World space outgoing vector for the shift.
};
...
public:
  ...
  void evaluatePoint(RadianceQueryRecord &rRec,
    ...
    evaluate(mainRay, shiftedRays, 4, very_direct, m_sensor->getMedium());
  ...
  inline BSDFSampleResult sampleBSDF(RayState &rayState, Float pdfComponent, int sampledComp) {
    ...
    result.bRec.component = sampledComp;

    Point2 sample = rRec.nextSample2D();
    result.weight = bsdf->sample(result.bRec, result.pdf, sample);

    // Update using the component choice...
    result.pdf *= pdfComponent; // We bake the pdfComponent inside it
    if (pdfComponent != 0.f) {
      result.weight /= pdfComponent;
    }

    // Variable result.pdf will be 0 if the BSDF sampler failed to produce a valid direction.
    //SAssert(result.pdf > (Float)0);
#if ORIGINAL_TESTS
    SAssert(fabs(result.bRec.wo.length() - 1.0) < 0.00001);
#else
    if (fabs(result.bRec.wo.length() - 1.0) >= 0.00001) {
      result.pdf = 0.0; // Invalid path
    }
#endif
  ...
  void evaluate(RayState &main,
                RayState *shiftedRays,
                int secondaryCount,
    ...
    bool mediumScattered = needRenderSurface(m_config->m_lightingInteractionMode);
    ...
    // We need to store
    NextSurfaceComponent nextSurfComponent;
...
//        }
      ...
      if (main.rRec.depth == 1) {
        ...
        if (med && med->sampleDistance(Ray(main.ray, 0, main.rRec.its.t), main.mRec, main.rRec.sampler)) {
          ...
          nextSurfComponent = NextSurfaceComponent();
          sampledMediumEvent = false;
          // If we have failed, we pre-select the BSDF component
          // Get in advance the BSDF selected component...
          const BSDF *mainBSDFAdvance = main.rRec.its.getBSDF(main.ray);
          Point2 randomBSDFSample = main.rRec.nextSample2D();
          BSDFSamplingRecord bRecAdvance(main.rRec.its, main.rRec.sampler);
          nextSurfComponent.component = mainBSDFAdvance->sampleComponent(bRecAdvance, nextSurfComponent.pdf,
                                                               randomBSDFSample,
                                                               0.05f);
//          bRecAdvance.component = nextSurfComponent.component;
//          nextSurfComponent.pdf = mainBSDFAdvance->pdfComponent(bRecAdvance);
          int componentQuery = (nextSurfComponent.component == -1 ? 0 : nextSurfComponent.component);
          nextSurfComponent.isRought = mainBSDFAdvance->getRoughness(main.rRec.its,
                                                         componentQuery) > 0.05f;
      ...
      if (sampledMediumEvent) {
        ...
        if (mediumScattered) {
          ...
          if (main.rRec.type & RadianceQueryRecord::EDirectMediumRadiance
              && main.rRec.depth + 1 >= m_config->m_minDepth && nbSurfaceInteractions >= m_config->m_minCameraDepth) {
            ...
            std::pair<Spectrum, bool> emitterTupleValue = m_scene->sampleAttenuatedEmitterDirectVisible(dRec,
                                                                                                        med,
                                                                                                        interactions,
                                                                                                        lightSample,
                                                                                                        main.rRec.sampler,
            ...
            for (int i = 0; i < secondaryCount; ++i) {
              ...
              if (shiftSuccessful) {
                ...
                if (shifted.connection_status == RAY_CONNECTED) {
                  ...
                  Float shiftedWeightDenominator = (jacobian * shifted.pdf) * (shiftedDRecPdf + shiftedPhasePdf);
                  ...
                  // TODO: As we did not sample the medium with shift path, we will use main phase function here
                  // Normally this strategy is Ok if the phase function parameter does not vary
                  // spatially
                  ...
                  VertexType
                      mainVertexType = VertexType::VERTEX_TYPE_DIFFUSE; //getVertexType(main, *m_config, BSDF::ESmooth);
                  VertexType shiftedVertexType =
                      VertexType::VERTEX_TYPE_DIFFUSE; //getVertexType(shifted, *m_config, BSDF::ESmooth);
                  ...
                  if (mainAtPointLight || (mainVertexType == VERTEX_TYPE_DIFFUSE
                    ...
                    std::pair<Spectrum, bool>
                        emitterTupleShiftValue = m_scene->sampleAttenuatedEmitterDirectVisible(shiftedDRec,
                                                                                          med,
                                                                                          interactions,
                                                                                          lightSample,
                                                                                          shifted.rRec.sampler, false);
                    bool shiftedEmitterVisible = emitterTupleShiftValue.second;
                    Spectrum shiftedEmitterRadiance = emitterTupleShiftValue.first * shiftedDRec.pdf;
                    ...
                    if (weight != 0) {
                      shiftedContribution = jacobian * shifted.throughput
                          * (shiftedPhaseValue * shiftedEmitterRadiance) / dRec.pdf;
                    }
                  }
              ...
              if (main.rRec.depth + 1 >= m_config->m_minDepth && nbSurfaceInteractions >= m_config->m_minCameraDepth) {
#if MEDIUM_DIRECT_TRANS
        ...
        // TODO: Always consider phase function as diffuse
        ...
        if (scene->rayIntersect(main.ray, main.rRec.its)) {
          sampledMediumEvent = med->sampleDistance(Ray(main.ray, 0, main.rRec.its.t), main.mRec, main.rRec.sampler);
...
//          if (!sampledMediumEvent) {
          ...
          if (main.rRec.its.isEmitter()) {
            ...
            mainEmitterRadiance *= med->evalTransmittance(Ray(main.ray, 0, main.rRec.its.t), main.rRec.sampler);
...
//          }
          ...
          mainNextVertexType = VertexType::VERTEX_TYPE_DIFFUSE;
          if (!sampledMediumEvent) {
            ...
            if (main.rRec.its.isValid()) {
              ...
              nextSurfComponent.component = mainBSDFAdvance->sampleComponent(bRecAdvance, nextSurfComponent.pdf,
                                                                   randomBSDFSample,
                                                                   0.05f);
//              bRecAdvance.component = componentSelected;
//              pdfComponent = mainBSDFAdvance->pdfComponent(bRecAdvance);
              int componentQuery = (nextSurfComponent.component == -1 ? 0 : nextSurfComponent.component);
              nextSurfComponent.isRought = mainBSDFAdvance->getRoughness(main.rRec.its,
                                                             componentQuery) > 0.05f;
              if (!nextSurfComponent.isRought) {
                // We cannot connect as the next surface is not rought
                // mark the mainNextVertexType accordinly
                mainNextVertexType = VERTEX_TYPE_GLOSSY;
              }
            }
          }
        ...
        const Float mainLumPdf = (mainHitEmitter && main.rRec.depth + 1 >= m_config->m_minDepth && nbSurfaceInteractions >= m_config->m_minCameraDepth) ?
        ...
        for (int i = 0; i < secondaryCount; ++i) {
          ...
          if (shifted.alive) {
            ...
            if (shifted.connection_status == RAY_CONNECTED) {
              ...
              // FIXME: In volume, we will might need to to half-vector copy.
              // FIXME: However, we use the phase function from the main path.
              // FIXME: This is valid only and only if the phase function parameter is spatially invariant.
...
//                  bool environmentConnection = false;
                  if (main.rRec.its.isValid()) {
                    ...
                    shiftSuccessReconnectionVolume.incrementBase();

                    // This is an actual reconnection shift.
                    if (!sampledMediumEvent) {
                      shiftResult = reconnectShift(m_scene,
                  ...
                  ++shiftSuccessReconnectionVolume;
...
//                        if (dot(shiftedDRec.n, -shiftedDRec.d) <= 0) {
//                          // Impossible to connect due to wrong orientation on the light source
//                          shiftedEmitterRadiance = Spectrum(0.f);
//                          shiftedLumPdf = 0.f;
//                        } else {
...
//                        }
                ...
                Spectrum shiftedEmitterRadiance(Float(0));
                shiftSuccessHVVolume.incrementBase();
#if ORIGINAL_TESTS
...
#endif
                {
...

                  {
                    ...
                    if (!scene->rayIntersect(shifted.ray, shifted.rRec.its)) {
                      ...
                      VertexType shiftedNextVertexType = VERTEX_TYPE_DIFFUSE;
                      if(!sampledMediumEvent) {
                        if(!shifted.rRec.its.isEmitter()) {
                          ...
                          // We can copy the vertex type now
                          shiftedNextVertexType = mainNextVertexType;
                        }
                      }
        ...
        if (mediumScattered) {
          ...
          if (main.rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance
              && mainBSDF->getType() & BSDF::ESmooth && main.rRec.depth + 1 >= m_config->m_minDepth && nbSurfaceInteractions >= m_config->m_minCameraDepth) {
            // Sample an emitter and evaluate f = f/p * p for it. */
            ...
            mainBRec.component = nextSurfComponent.component;
            // Evaluate BSDF * cos(theta).
            Spectrum mainBSDFValue = mainBSDF->eval(mainBRec);
            mainBSDFValue /= nextSurfComponent.pdf;

            // Calculate the probability density of having generated the sampled path segment by BSDF sampling. Note that if the emitter is not visible, the probability density is zero.
            // Even if the BSDF sampler has zero probability density, the light sampler can still sample it.
            Float mainBsdfPdf =
                (emitter->isOnSurface() && dRec.measure == ESolidAngle && mainEmitterVisible)
                ? mainBSDF->pdf(mainBRec) * nextSurfComponent.pdf : 0;
            ...
            // Power heuristic weights for the following strategies: light sample from base, BSDF sample from base.
            Float mainWeightNumerator = main.pdf * dRec.pdf;// * nextSurfComponent.pdf;
                ...
                if (shiftSuccessful) {
                  ...
                  if (shifted.connection_status == RAY_CONNECTED) {
                    ...
                    BSDFSamplingRecord bRec(main.rRec.its,
                                            ...
                                            ERadiance);
                    bRec.component = nextSurfComponent.component;

                    // Sample the BSDF.
                    Float shiftedBsdfPdf = (emitter->isOnSurface() && dRec.measure == ESolidAngle
                        && mainEmitterVisible) ? mainBSDF->pdf(bRec) * mainBSDF->pdfComponent(bRec)
                                               : 0; // The BSDF sampler can not sample occluded path segments.
                    Float shiftedDRecPdf = dRec.pdf;
                    Spectrum shiftedBsdfValue = mainBSDF->eval(bRec) / nextSurfComponent.pdf;
                    Spectrum shiftedEmitterRadiance = mainEmitterRadiance;
                    auto jacobian = (Float) 1;
                    ...
                    VertexType mainVertexType = nextSurfComponent.isRought ? VERTEX_TYPE_DIFFUSE : VERTEX_TYPE_GLOSSY;
                    VertexType shiftedVertexType = mainVertexType;
                    if(shiftedBSDF != mainBSDF) {
                      // Let's assume in this case that we have a glossy type
                      shiftedVertexType = VERTEX_TYPE_GLOSSY;
                    }
                    ...
                    if (mainAtPointLight || (mainVertexType == VERTEX_TYPE_DIFFUSE
                      ...
                      // Evaluate the BSDF, where we assuming that shiftedDRec and dRec store the same
                      // light source sampled point.
                      Float shiftedDistanceSquared =
                      ...
                      if (m_config->m_strictNormals
                        ...
                        Spectrum shiftedBsdfValue = shiftedBSDF->eval(bRec) / nextSurfComponent.pdf;
                        Float shiftedBsdfPdf =
                            (emitter->isOnSurface() && dRec.measure == ESolidAngle
                                && shiftedEmitterVisible) ? shiftedBSDF->pdf(bRec) * shiftedBSDF->pdfComponent(bRec): 0;
                        Float jacobian = std::abs(shiftedOpposingCosine * mainDistanceSquared)
                            / (Epsilon + std::abs(
                                mainOpposingCosine * shiftedDistanceSquared));
                        ...
                        if (weight != 0.f) {
                          shiftedContribution = jacobian * shifted.throughput
                              * (shiftedBsdfValue * shiftedEmitterRadiance) / dRec.pdf;
                        }
                      }
                    }
                  }
        ...
        // Sample a new direction from BSDF * cos(theta).
        BSDFSampleResult mainBsdfResult = sampleBSDF(main, nextSurfComponent.pdf, nextSurfComponent.component);
        ...
        int previousComponentSelected = nextSurfComponent.component;
        ...
        VertexType mainVertexType = nextSurfComponent.isRought ? VERTEX_TYPE_DIFFUSE : VERTEX_TYPE_GLOSSY;
        VertexType mainNextVertexType;
        if (nextSurfComponent.isRought && (!mediumScattered)) {
          return; // Stop here
        }
          ...
          if (shifted.alive) {
            ...
            if (shifted.connection_status == RAY_CONNECTED) {
              ...
              BSDFSamplingRecord bRec(previousMainIts,
                                      ...
                                      ERadiance);
              bRec.component = previousComponentSelected;
              ...
              if(shifted.rRec.its.isMediumTransition()) {
                ...
                shiftFailedVolume.incrementBase();
                if (med != shiftMed) {
                  ++shiftFailedVolume;
              ...
              if (mainVertexType == VERTEX_TYPE_DIFFUSE && mainNextVertexType == VERTEX_TYPE_DIFFUSE
                ...
                if (!lastSegment || mainHitEmitter || main.rRec.its.hasSubsurface()) {
                  ReconnectionShiftResult shiftResult;
//                  bool environmentConnection = false;
                  shiftSuccessReconnectionSurface.incrementBase();
...
//                    shiftResult = environmentShift(m_scene, main.ray, shifted.rRec.its.p);
                  ...
                  ++shiftSuccessReconnectionSurface;
                  shifted.connection_status = RAY_RECENTLY_CONNECTED;
                  ...
                  if (mainHitEmitter || main.rRec.its.hasSubsurface()) {
                    ...
                    if (main.rRec.its.isValid()) {
                      ...
                      if (mainHitEmitter) {
                        ...
                        if(med) {
                          shiftedEmitterRadiance *= med->evalTransmittance(Ray(shifted.rRec.its.p,
                                                                               shiftedDRec.d,
                                                                               0,
                                                                               shiftedDRec.dist,
                                                                               shifted.mRec.time),
                ...
                if (!(bothDelta || bothSmooth)) {
                  ++HVSurfaceDisagreement;
                  shifted.alive = false;
                  goto half_vector_shift_failed;
                }

                // FIXME: FIXME FIXME FIXME FIXME FIXME FIXME
                // FIXME: Need to be restrictive to the BSDF component sampled
#if ORIGINAL_TESTS
                SAssert(fabs(shifted.ray.d.lengthSquared() - 1) < 0.000001);
#else
                if (fabs(shifted.ray.d.lengthSquared() - 1) >= 0.000001) {
...
#endif
                {
                  ...
                  HalfVectorShiftResult shiftResult = halfVectorShift(mainBsdfResult.bRec.wi,
                                                ...
                                                mainBSDF->getEta(),
                                                shiftedBSDF->getEta());
                  shiftSuccessHVSurface.incrementBase();
                  ...
                  if (mainBSDF != shiftedBSDF) {
                    shifted.alive = false;
                    goto half_vector_shift_failed;
                  }

                  ++shiftSuccessHVSurface;

                  // Update the vertex type.
                  // As we are sure that it is the same BSDF, we can do the shift
                  shiftedVertexType = mainVertexType;//getVertexType(shifted, *m_config, mainBsdfResult.bRec.sampledType);
    ...
    for (int i = 0; i < secondaryCount; ++i) {
      RayState &shift = shiftedRays[i];
      shiftDied.incrementBase();
      if(!shift.alive) {
        ++shiftDied;
      }
    }
...
GradientPathIntegrator::GradientPathIntegrator(const Properties &props)
  ...
  */
  ...
  m_dumpIteration = props.getInteger("dumpIteration", 0);
...
void GradientPathIntegrator::renderBlock(const Scene *scene,
                                         ...
                                         GPTWorkResult *block,
                                         const bool &stop,
                                         const std::vector<TPoint2<uint8_t>
                                         > &points) const {
...
// Get the initial ray to sample.
      rRec.
          newQuery(RadianceQueryRecord::ESensorRay, sensor
          ->
              getMedium()
      );
...
// Add the throughput image as a preview. Note: Preview and final buffers are shared.
      {
        ...
        block->
            put(samplePos,
                (
                    8 * centralVeryDirect) + (2 * centralThroughput),
                4.0f,
                4.0f,
        ...
        block->
            put(left_pixel, (
            2 * shiftedThroughputs[LEFT]), 1.0f, 1.0f, BUFFER_FINAL);     // Negative x throughput.
        block->
            put(right_pixel, (
            2 * shiftedThroughputs[RIGHT]), 1.0f, 1.0f, BUFFER_FINAL);   // Positive x throughput.
        block->
            put(top_pixel, (
            2 * shiftedThroughputs[TOP]), 1.0f, 1.0f, BUFFER_FINAL);       // Negative y throughput.
        block->
            put(bottom_pixel, (
            2 * shiftedThroughputs[BOTTOM]), 1.0f, 1.0f, BUFFER_FINAL); // Positive y throughput.
#endif
      }
...
// This can be replaced with a standard throughput sample without much loss of quality in most cases.
      {
        ...
        block->
            put(samplePos, (
            2 * centralThroughput), 4.0f, 4.0f, BUFFER_THROUGHPUT); // Central throughput.

        block->
            put(left_pixel,
                (
                    2 * shiftedThroughputs[LEFT]),
                1.0f,
                1.0f,
...
// Very direct.
      block->
          put(center_pixel, centralVeryDirect,
              1.0f, 1.0f, BUFFER_VERY_DIRECT);
    }
  }
}

void averageBuffer(const Vector2i &cropSize, Bitmap *accum, Bitmap *iter, int iterationNumber) {
  // copy abs gradient values
  auto *accumPtr = (Spectrum *) accum->getUInt8Data();
  auto *iterPtr = (Spectrum *) iter->getUInt8Data();
  for (int y = 0; y < cropSize.y; ++y) {
    for (int x = 0; x < cropSize.x; ++x) {
      int pixelID = y * cropSize.x + x;
      accumPtr[pixelID] = (accumPtr[pixelID] * (iterationNumber - 1) + iterPtr[pixelID]) / iterationNumber;
    }
  }
}
...
bool GradientPathIntegrator::render(Scene *scene,
  ...
  Log(EInfo, "Starting render job (GPT::render) (%ix%i, "
      SIZE_T_FMT
      " %s, "
      SIZE_T_FMT
      " %s, "
  ...
  // TODO: to agree with Adrien about the inifinite loop here.
  while (true) {
    ...
    // Average results from past iterations
    /* Allocate bitmaps for the solvers. */
    ref<Bitmap> throughputBitmapIter(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
    ref<Bitmap> directBitmapIter(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
    ref<Bitmap> dxBitmapIter(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));
    ref<Bitmap> dyBitmapIter(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));

    /* Develop primal and gradient data into bitmaps. */
    film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), throughputBitmapIter, BUFFER_THROUGHPUT);
    film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dxBitmapIter, BUFFER_DX);
    film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), dyBitmapIter, BUFFER_DY);
    film->developMulti(Point2i(0, 0), film->getCropSize(), Point2i(0, 0), directBitmapIter, BUFFER_VERY_DIRECT);

    /* Average the results of other iterations */
    averageBuffer(cropSize, throughputBitmap, throughputBitmapIter, currentIteration);
    averageBuffer(cropSize, dxBitmap, dxBitmapIter, currentIteration);
    averageBuffer(cropSize, dyBitmap, dyBitmapIter, currentIteration);
    averageBuffer(cropSize, directBitmap, directBitmapIter, currentIteration);

#ifdef RECONSTRUCT
    /* Reconstruct. */
    int virtualIter = currentIteration * m_iterationMult;
    if (m_dumpIteration > 0 && virtualIter % m_dumpIteration == 0) {
      // Just allocate the reconstruction buffer
      ref<Bitmap> reconstructionBitmap(new Bitmap(Bitmap::ESpectrum, Bitmap::EFloat, film->getCropSize()));

      /* Prepare and write data on the disk */
      // Develop the two buffers
      // Create hdr film to avoid to dump several buffers
      ...
      develop(scene, hdrFilm, throughputBitmap, virtualIter, "_throughput_");
      develop(scene, hdrFilm, directBitmap, virtualIter, "_direct_");
      ...
      auto subPixelCount = size_t(3 * film->getCropSize().x * film->getCropSize().y);
      ...
      std::transform(dxBitmap->getFloatData(),
                     dxBitmap->getFloatData() + subPixelCount,
                     dxVector.begin(),
                     [](Float x) { return (float) x; });
      std::transform(dyBitmap->getFloatData(),
                     dyBitmap->getFloatData() + subPixelCount,
                     dyVector.begin(),
                     [](Float x) { return (float) x; });
      ...
      paramsL1.alpha = (float) m_config.m_reconstructAlpha;
      ...
      if (m_config.m_reconstructL2) {
        ...
        paramsL2.alpha = (float) m_config.m_reconstructAlpha;
        ...
        solverL2.importImagesMTS(dxVector.data(),
                                 dyVector.data(),
        ...
        for (int y = 0, p = 0; y < h; ++y) {
          for (int x = 0; x < w; ++x, p += 3) {
            ...
            color[0] = std::max(Float(0.0f), color[0]);
            color[1] = std::max(Float(0.0f), color[1]);
            color[2] = std::max(Float(0.0f), color[2]);

            reconstructionBitmap->setPixel(Point2i(x, y), Spectrum(color));
            if (m_config.forceBlackPixels &&
                throughputVector[p] == 0.0f &&
                throughputVector[p + 1] == 0.0f &&
                throughputVector[p + 2] == 0.0f &&
                directVector[p] == 0.0f && directVector[p + 1] == 0.0f && directVector[p + 2] == 0.0f) {
              reconstructionBitmap->setPixel(Point2i(x, y), Spectrum(0.0f));
            }
          }
        }

        if (m_config.m_reconstructL1) {
          develop(scene, hdrFilm, reconstructionBitmap, virtualIter, "_L2_");
        } else {
          develop(scene, hdrFilm, reconstructionBitmap, virtualIter, "_recons_");
        }
      ...
      if (m_config.m_reconstructL1) {
        ...
        if (m_config.m_reconstructL2) {
          develop(scene, hdrFilm, reconstructionBitmap, virtualIter, "_L1_");
        } else {
          develop(scene, hdrFilm, reconstructionBitmap, virtualIter, "_recons_");
        }
      ...
      if (m_showAbsGradients) {
        ...
        develop(scene, hdrFilm, dxBitmapAbs, virtualIter, "_dxAbs_");
        develop(scene, hdrFilm, dyBitmapAbs, virtualIter, "_dyAbs_");
