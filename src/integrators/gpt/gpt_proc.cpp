...
public:
	...
	void prepare() override {
		Scene *scene = dynamic_cast<Scene *>(getResource("scene"));
		m_scene = new Scene(scene);
		m_sampler = dynamic_cast<Sampler *>(getResource("sampler"));
		m_sensor = dynamic_cast<Sensor *>(getResource("sensor"));
		m_integrator = dynamic_cast<GradientPathIntegrator *>(getResource("integrator"));
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult,
		...
		const auto *rect = dynamic_cast<const RectangularWorkUnit *>(workUnit);
		auto *block = dynamic_cast<GPTWorkResult *>(workResult);
...
protected:
  ~GPTBlockRenderer() override = default;
...
void GPTRenderProcess::processResult(const WorkResult *result, bool cancelled) {
	const auto * block = dynamic_cast<const GPTWorkResult *>(result);
