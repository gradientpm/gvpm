...
public:
	MIPathTracer(const Properties &props)
		: MonteCarloIntegrator(props) {
		  m_coherent = props.getBoolean("coherent", false);
      m_comp = props.getInteger("comp", 0);
		}
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        if(bsdf->getRoughness(its, bRec.sampledComponent) > 0.05f) {
          // Vector initisation to query comp
          Vector vec(0.f);
          vec[m_comp] = 1.f;
