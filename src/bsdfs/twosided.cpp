...
public:
	...
	void configure() {
		...
		// FIXME: This is a very strong assumption but it really simplify the implementation
		// Assumption: The BSDF for the two side is the same
		if(m_nestedBRDF[0] != m_nestedBRDF[1]) {
			SLog(EError, "Break the assumption to have the same BSDF.");
		}
	...
	Spectrum eval(const BSDFSamplingRecord &bRec, EMeasure measure) const {
		...
		if (Frame::cosTheta(b.wi) > 0) {
			return m_nestedBRDF[0]->eval(b, measure);
		  	...
		  	// FIXME: This is a dirty fix but may work for the deadline
			//if (b.component != -1)
	...
	Float getRoughness(const Intersection &its, int component) const {
		if (component < m_nestedBRDF[0]->getComponentCount()) {
			...
			return m_nestedBRDF[1]->getRoughness(its, component // FIXME
	...
	Float pdfComponent(const BSDFSamplingRecord& bRec) const {
	  if (Frame::cosTheta(bRec.wi) < 0) {
		  ...
		  // FIXME
          //if (bRecFlipped.component != -1)
          //    bRecFlipped.component -= m_nestedBRDF[0]->getComponentCount();

	      return m_nestedBRDF[1]->pdfComponent(bRecFlipped);
      }
      else {
          return m_nestedBRDF[0]->pdfComponent(bRec);
      }
	}
