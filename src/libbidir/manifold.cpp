...
bool SpecularManifold::init(const Path &path, size_t start, size_t end) {
	...
	if (vs->getType() & (PathVertex::ESensorSample | PathVertex::EEmitterSample)) {
		...
		if (type == AbstractEmitter::EDeltaDirection) {
			...
			v.gn = v.n = pRec.n;
			coordinateSystemCoherent(pRec.n, v.dpdu, v.dpdv);
		}
	}
	...
	for (size_t i=start + step; i != end; i += step) {
		...
		if (vertex->isSurfaceInteraction()) {
			...
			Vector s, t;
			coordinateSystemCoherent(wi, s, t);
...
bool SpecularManifold::move(const Point &target, const Normal &n,
	...
	coordinateSystemCoherent(n, last.dpdu, last.dpdv);
	last.n = n;
...
bool SpecularManifold::update(const Path &source, Path &path, size_t start, size_t end, bool adjointCorr, bool gvpm) {
	int step;
	ETransportMode mode;
...
//			PathVertex::EMediumInteraction : PathVertex::ESurfaceInteraction;
        PathVertex::EVertexType desiredType = source.vertex(i+step)->getType();
		...
		if (v.type == EPinnedDirection) {
			...
			if (!vertex->perturbDirection(m_scene,
					...
					length, desiredType, mode, adjointCorr)) {
				#if MTS_MANIFOLD_DEBUG == 1
