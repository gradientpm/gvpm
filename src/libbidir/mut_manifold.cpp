...
ManifoldPerturbation::ManifoldPerturbation(const Scene *scene, Sampler *sampler,
		  ...
		  Float avgAngleChangeMedium, Float specularThreshold, int maxManifoldIterations) : 
		  m_scene(scene),
	...
	m_manifold = new SpecularManifold(scene, maxManifoldIterations);
	...
	// Not needed anymore, only for debug proposes
//    Statistics &stats = *Statistics::getInstance();
//    stats.registerCounter(&statsMWFailInit);
//    stats.registerCounter(&statsMWZeroLength);
//    stats.registerCounter(&statsMWFailMove);
//    stats.registerCounter(&statsMWFailUpdate);
//    stats.registerCounter(&statsMWFailRobustness);
//    stats.registerCounter(&statsMWFailReversible);

}

ManifoldPerturbation::~ManifoldPerturbation() = default;
...
bool ManifoldPerturbation::sampleMutationRecord(
	...
	return (c = getSpecularChainEnd(source, b + step, step)) != -1;
}
...
bool ManifoldPerturbation::sampleMutation(
	...
	if (a != 0 && a != k) {
		...
		*succEdge = *succEdge_old;
		...
		vertex->update(m_scene, nullptr, succ, mode, succ_old->measure);
	}
	...
	for (int i = a + step; i != b; i += step) {
		...
		if (vertex_old->isSurfaceInteraction()) {
			...
			if (vertex_old->isConnectable()) {
				Vector mV(0.0f);
				if (reflection)
					mV = normalize(wi_old + wo_old);
				else if (eta != 1)
					mV = normalize(wi_old.z < 0 ? (wi_old*eta + wo_old)
						: (wi_old + wo_old*eta));
				mV = its_new.toWorld(mV.z > 0 ? mV : -mV);

				if (reflection) {
					wo_world = reflect(wi_world, mV);
				} else {
					if (eta != 1) {
						wo_world = refract(wi_world, mV, eta);
						if (wo_world.isZero())
							goto fail;
					} else {
						wo_world = -wi_world;
	...
	if (std::abs(b-c) > 1) {
		...
		if (!m_manifold->move(vb_new->getPosition(), n))
			goto fail;
		if (!m_manifold->update(source, proposal, c, b))
			goto fail;
...
Float ManifoldPerturbation::Q(const Path &source, const Path &proposal,
	...
	auto mode = (ETransportMode) muRec.extra[4];
    ...
    // Equation 12 in manifold technical report
	weight *=
...
bool ManifoldPerturbation::generateOffsetPathGBDPT(Path &source, Path &proposal, MutationRecord &muRec, Vector2 offset, bool &couldConnectBehindB, bool lightPath) {
	...
	proposal.edge(proposal.edgeCount() - 1)->medium = source.edge(proposal.edgeCount() - 1)->medium; // Copy proposal
	...
	for (int i = v_start; i < m; ++i) {
		...
		memset(proposal.edge(proposal.edgeCount() - 1), 0, sizeof(PathEdge));  // make sure edges are ALWAYS initialized!
		proposal.edge(proposal.edgeCount() - 1)->medium = source.edge(proposal.edgeCount() - 1)->medium;
	}
	proposal.append(source, m, k + 1); //[m...k] stays the same

	//make deep copy of a and c and 
	proposal.vertex(a) = source.vertex(a)->clone(m_pool);
	proposal.vertex(c) = source.vertex(c)->clone(m_pool);


	/* Sample the first vertex */
	if (!perturbDirection(source, proposal, step, a, offset, mode)){
		goto FailMut; //fails when it misses the scene
	}
	...
	if (std::abs(b - c) > 1) {
		...
		if (!walkSuccess){
			for (int i = b + step; i != c; i += step){
				m_pool.release(proposal.edge(i - 1));
				m_pool.release(proposal.vertex(i));
				proposal.vertex(i) = source.vertex(i)->clone(m_pool);
				proposal.edge(i - 1) = source.edge(i - 1)->clone(m_pool);
			}
			muRec.extra[2] = b + step;
		}
	}

	// Just make sure that the index are the same
	if(proposal.vertexOrNull(q-1) != nullptr) {
		proposal.vertexOrNull(q-1)->sampledComponentIndex = source.vertexOrNull(q-1)->sampledComponentIndex;
	}
	if(proposal.vertex(q) != nullptr) {
		proposal.vertex(q)->sampledComponentIndex = source.vertex(q)->sampledComponentIndex;
	}
	if(proposal.vertex(q+1) != nullptr) {
		proposal.vertex(q+1)->sampledComponentIndex = source.vertex(q+1)->sampledComponentIndex;
	}
	if(proposal.vertexOrNull(q+2) != nullptr) {
		proposal.vertexOrNull(q+2)->sampledComponentIndex = source.vertexOrNull(q+2)->sampledComponentIndex;
	}
...
bool ManifoldPerturbation::perturbDirection(Path &source, Path &proposal, int step, int a, Vector2 offset, ETransportMode mode)
{
	const PathVertex *succ_old = source.vertex(a + step);
	const PathEdge
		*succEdge_old = source.edge(mode == EImportance ? a : a - 1);
	...
	const auto *sensor = dynamic_cast<const PerspectiveCamera *>(m_scene->getSensor());
	...
	//======>
	return vertex->perturbDirection(m_scene,
		pred, predEdge, succEdge, succ, wo_new,
		succEdge_old->length, succ_old->getType(), mode);
}
...
bool ManifoldPerturbation::propagatePerturbation(Path &source, Path &proposal, int step, int a, int b, ETransportMode mode)
{
	...
	for (int i = a + step; i != b; i += step) {
		...
		if (vertex_old->isSurfaceInteraction()) {
			...
			bool reflection = Frame::cosTheta(wi_old) * Frame::cosTheta(wo_old) > 0;
			Float eta = vertex_old->getIntersection().getBSDF()->getEta();
			Vector wi_world = normalize(pred->getPosition() - vertex->getPosition()), wo_world(0.0f);
			...
			if (vertex_old->isConnectable()) {
				...
				if (reflection) {
					...
					m = normalize(wi_old.z < 0 ? (wi_old*eta + wo_old) : (wi_old + wo_old*eta));
                }
...
bool ManifoldPerturbation::manifoldWalk(Path &source, Path &proposal, int step, int b, int c)
{
	...
	if (!vb_old->isMediumInteraction()) {
		...
		n1 = normalize(vb_old->getPosition() - source.vertex(b - step)->getPosition());
		n2 = normalize(vb_new->getPosition() - proposal.vertex(b - step)->getPosition());  // step = -1, c < b < a, so n2 points from a towards b
	}
...
int ManifoldPerturbation::getSpecularChainEndGBDPT(const Path &path, int pos, int step) {
	while (true) {
		...
		int component = vertex->sampledComponentIndex;
		if(component == -1) component = 0;

		roughness = bsdf->getRoughness(its, component);
...
bool ManifoldPerturbation::computeMuRec(Path &source, MutationRecord &muRec) {
...
bool ManifoldPerturbation::manifoldWalkGPM(const Path &source, Path &proposal,
										   int step, size_t b, size_t c, Float kernelRadius,
										   Point gpBasePos, Point gpOffsetPos)
{
    ...
    const PathVertex
        *vb_old = source.vertex(c),
        *vb_new = proposal.vertex(c);
    ...
    if (!vb_old->isMediumInteraction()) {
        ...
        // flow from light source
        n1 = normalize(vb_old->getPosition() - source.vertex(c - step)->getPosition());        
        // here step = 1, b < c, and we do not have the vertices from b+1 to c-1 yet,
        // so we approximate the neighbor vertex using c-1 on the source path
        n2 = normalize(vb_new->getPosition() - source.vertex(c - step)->getPosition());
    }
    ...
    // TODO: using n1 and n2 instead of n seems ok. Do we really need n?

    if (!m_manifold->init(source, b, c)) {  // Start vertex is pinned. End vertex is movable
        ++statsMWFailInit;
        return false;
    }
    p0 = m_manifold->getPosition(1);  // The first specular vertex
    if (!m_manifold->move(vb_new->getPosition(), n, kernelRadius, gpOffsetPos)){
    //if (!m_manifold->move(vb_new->getPosition(), n2)){
        ++statsMWFailMove;
        return false;
    }
    // Update excluding the last two vertices, have to manually compute their pdf and contribution later
    if (!m_manifold->update(source, proposal, b, c, false, true)){
        ++statsMWFailUpdate;
        return false;
    }
    if (!m_manifold->move(vb_old->getPosition(), n, kernelRadius, gpBasePos)) {   // Reversible check
    //if (!m_manifold->move(vb_old->getPosition(), n1)) {
        ++statsMWFailReversible;
        return false;
    }

    // Make sure the first specular vertex matches, which leads to same path and hence reversible
    if (kernelRadius == 0.f) {
        Point p1 = m_manifold->getPosition(1);       
        Float relerr = (p0 - p1).length() / m_scene->getBSphere().radius;
        if (relerr > 10.0f * MTS_MANIFOLD_EPSILON) {
...
//        Point p1 = m_manifold->getPosition(1);
//        Float distBase = (gpBasePos - p1).length();
//		Float distOffset = (gpOffsetPos - p0).length();
//        // Relax this check with kernel radius
//        if (distBase > kernelRadius || distOffset > kernelRadius) {
//            ++statsMWFailRobustness;
//            return false;
//        }
    }
