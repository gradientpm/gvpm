...
Float PathEdge::evalPdf(const PathVertex *pred,
    const PathVertex *succ, Float t, bool succVolume) const
{
    // If the next vertex is a medium vertex, point at distance t only appears
    // when the edge length is greater than t
    // See Eq. 53 in Krivanek et al.'s UPBP paper.
    ...
    Float lengthPoint = d.length();
    Ray ray(a, d / lengthPoint, 0, lengthPoint, pred->getTime());
